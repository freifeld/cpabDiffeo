...
extern "C"{
__global__ void expm(double* As,double* Ts,
                 int n, /* Each of the As is n x (n+1)  */
                 int N, /* Number of As                 */
...
def gpu_expm(As,Ts_vectorized,n,p):   
    nElts = n * (n+1)
    N=len(As) 
    if Ts_vectorized.ndim != 2 or Ts_vectorized.shape[1] != nElts:
...
if __name__ == "__main__":
    ...
    n = 4
    As = CpuGpuArray.zeros((N,n,n+1))
    Ts = CpuGpuArray.zeros((N,n*(n+1)))
...
#    print As2
    ...
    if use_cy:
        tic = time.clock()
        expm_affine_ND_multiple(As.cpu,Ts_scipy)
        toc = time.clock()
