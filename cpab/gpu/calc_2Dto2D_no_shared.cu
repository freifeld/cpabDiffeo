...
__device__ inline void const_A_times_b(double x[], const double A[], double b[])
{
// Result is computed inside x. 
  x[0] = A[0]*b[0] + A[1]*b[1] + A[2];
  x[1] = A[3]*b[0] + A[4]*b[1] + A[5]; 
...
#pragma unroll
    ...
    // compute velocity at mid point
    const_A_times_b(vMid,As+mi,pMid);
...
#pragma unroll
        ...
        // Term1
        const_A_times_b(B_times_T,Bs+ bi   , p); 
        // Term2
        const_A_times_b(A_times_dTdAlpha,As+mi  , q); 
        // Sum both terms 
#pragma unroll
        for(int i=0; i<DIM; ++i){           
...
#pragma unroll
        ...
        // Term1
        const_A_times_b(B_times_T,Bs+ bi  , pMid);        
        // Term2
        const_A_times_b(A_times_dTdAlpha,As+mi  , qMid); 
        // Sum both terms 
#pragma unroll
        for(int i=0; i<DIM; ++i){           
...
#pragma unroll
    ...
    {
      cell_idx = compute_cell_idx(p,nC0,nC1,nC2,inc_x,inc_y);                
      const_A_times_b(pNew,Trels + cell_idx*DIM*(DIM+1),p);
...
#pragma unroll
    ...
    const_A_times_b(v,As + cell_idx*DIM*(DIM+1),p);  
